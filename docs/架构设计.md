...
┌───────────────┴───────────────────────────────────────────────────────────┐
│ ⑤ 运行时与分发                                                            │
│   Tauri 2.0（Windows / Android；WebView2 或系统 WebView）                  │
│   PWA（iOS Safari、HarmonyOS ArkWeb，manifest + service worker）           │
...
| Tauri 2.0 | 2.x | 安装包小（非 Electron）、支持移动端、可访问文件系统 | Electron：安装包 80MB+，违背「小巧」定位 |
| PWA | 原生 | 覆盖 iOS/HarmonyOS 等无法打包的平台，零分发成本 | 原生多端：v0.1 成本不可控 |
