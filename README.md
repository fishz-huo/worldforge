...
| 5 | 插入图片 | `src/features/cards/CardGallery.tsx` | 元数据入 SQLite、二进制入 IndexedDB；一张卡片多张图、可设封面；正文里可直接粘贴/拖入图片（`asset:` 引用） |
...
| 14 | 插件区与插件插口 | `src/features/plugins/` + `src/lib/plugin/` | 7 类插口（卡片类型、追加字段、面板、命令、卡片动作、主题、导出器）+ 事件订阅 + 只读查询 + 私有存储；附 3 个可读可改的内置示例插件 |
...
scripts/            图标生成 + 7 个自测套件（运行器、解析钩子、假 IndexedDB、selector 与表描述静态检查器，均零依赖）
...
通过模块解析钩子（`scripts/alias-hook.mjs`）把 `@/` 别名与 Vite 的 `?url` 资源导入补齐，
因此无需浏览器即可端到端验证数据层。
...
代码规范自查：176 个源文件中**没有任何文件超过 200 行**（`README`/文档除外），
...
| [`docs/插件开发指南.md`](docs/插件开发指南.md) | PluginAPI 逐方法说明、事件与设置 schema、3 个完整可运行示例、发布与分享 |
