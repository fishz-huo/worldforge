...
**这是一个「自己用」的工具，顺便开源，请先读完这一节再决定是否使用。**

- **它诞生于 vibe coding。** 作者不是程序员，看不懂这个仓库里的代码。全部代码与文档
  都是在与 AI 模型（DeepSeek）的对话中生成、由作者提出需求并反复试用后定下来的。
  把它公开，是因为作者觉得可能有人用得上，**不是**因为作者能对它的质量负责。
- **作者没有能力提供技术支持。** 这个项目很可能**长期不更新**；作者看不懂代码，
  因此无法解答「为什么这样写」「怎么改成我要的样子」这类问题，也不保证会回复 issue。
  以后作者若突发奇想要加点什么，才会动它一次。
- **没有任何担保。** 软件按「现状」提供，不担保它能满足你的任何特定用途、
  不担保它没有缺陷、**不担保你的数据不会丢失**。请**务必定期使用
  「设置 → 数据 → 导出完整备份」**，并把备份文件放到别处。
- **作者不是法律意义上的责任人。** 使用本软件产生的一切后果由使用者自行承担。
  完整的免责条款见 [`LICENSE`](LICENSE)（MIT 协议中的全大写段落）。

> 如果你想要一个有人维护、能提问、能保证稳定的世界观工具，这个项目**不适合你**。
> 如果你只是想找个本地优先、不联网、能自己改的小工具，欢迎拿走用。

## 反馈与贡献

作者欢迎别人来改它、修它、加功能 —— 但**不是**直接改这个仓库。

- **别人无法直接修改本仓库。** GitHub 上除作者外没有人有推送权限。
  想让改进进入本项目，走 **Pull Request**：先 fork 到你自己的账号 → 改完 → 提 PR →
  作者有空时看一眼再决定合并。作者看不懂代码，**很可能合并得很慢甚至不合并**，
  这是这个项目的常态，请别介意。
- **其实你不需要等作者。** MIT 协议允许你自由使用、修改、分发这份代码，
  fork 之后完全按自己的想法改就好，想叫别的名字、想闭源、想商用都行（保留版权声明即可）。
- **写插件是最推荐的贡献方式**，因为它**不需要动主程序**：把一个 `.js` 文件放进
  `plugins/` 目录就能加载。方法见
  [`docs/插件开发指南.md`](docs/插件开发指南.md)，仓库里的 `plugins/*.js`
  （图库灯箱、文档导出）就是可直接照抄的完整范例。
- **发现了 bug 又不会改？** 开个 issue 描述「点了什么 → 期望什么 → 实际发生了什么」
  即可，附上截图更好。作者不保证能修，但看到了会记下来。
